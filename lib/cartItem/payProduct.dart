...
class viewpayProduct extends State<payProduct> {
  ...
  void addBuyProduct(Map<String, dynamic> selectedAddress)async{
    ...
    for (int i = 0; i < widget.data.length; i++) {
      ...
      int price =int.parse(getData["price"].toString().replaceAll(".",""))*int.parse(getData["quantity"]);
      String formattedTotalPayment = NumberFormat.decimalPattern("vi").format(price);
      ...
      final newDocument = firestore.collection("User").doc(itemUser.data[0]["key"]).collection("Purchasehistory").doc();
      newDocument.set({
        "productKey": newDocument.id,
                  ...
                  child: Column(
                    children: [
                      ...
                      ListView.builder(
                        ...
                        itemCount: 1,
                        itemBuilder: (context, index) {
                          final getItem= itemUser.addresses[0];
                          final getData= getItem.value;
                        return itemUser.addresses.isNotEmpty? Card(
                            child: Container(
                              padding:
